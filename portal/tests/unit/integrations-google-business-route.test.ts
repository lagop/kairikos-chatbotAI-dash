// =============================================================================
// Unit tests for /api/admin/portal/settings/integrations/google-business.
// Mirrors integrations-google-places-route.test.ts's conventions closely,
// with a clientId + clientSecret body instead of a single apiKey.
// =============================================================================
...
import { GET, POST } from '@/app/api/admin/portal/settings/integrations/google-business/route';
...
describe('GET /api/admin/portal/settings/integrations/google-business', () => {
  ...
  it('returns the masked credential status, including the cleartext clientId', async () => {
    ...
    expect(mockState.getIntegrationCredentialStatus).toHaveBeenCalledWith('google_business');
  });
...
describe('POST /api/admin/portal/settings/integrations/google-business', () => {
  const VALID_BODY = { clientId: '123-abc.apps.googleusercontent.com', clientSecret: 'gocspx-real-looking-secret' };
  ...
  it('saves under toolKey "google_business" with the resolved operator email', async () => {
    const res = await POST(makeRequest(VALID_BODY));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ ok: true, clientId: VALID_BODY.clientId, lastFour: 'cret' });
  ...
  it('never calls a real verification API — a wrong pair is only caught on the first client OAuth attempt', async () => {
    const res = await POST(makeRequest(VALID_BODY));
    expect(res.status).toBe(200);
  });
});
...
describe('TOTP reciente (seguridad, 22/09/2026)', () => {
  it('sin TOTP reciente no deja cambiar el cliente OAuth de Google Business: 403 totp_step_up_required', async () => {
    stepUpState.requireTotpStepUp.mockResolvedValueOnce({ ok: false, status: 403, error: 'totp_step_up_required' });
    const res = await POST(makeRequest({ clientId: '123-abc.apps.googleusercontent.com', clientSecret: 'gocspx-real-looking-secret' }));
