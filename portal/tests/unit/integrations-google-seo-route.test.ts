// =============================================================================
// Unit tests for /api/admin/portal/settings/integrations/google-seo.
...
import { GET, POST } from '@/app/api/admin/portal/settings/integrations/google-seo/route';
...
describe('GET /api/admin/portal/settings/integrations/google-seo', () => {
  ...
  it('returns the masked credential status, including the cleartext clientId', async () => {
    mockState.getIntegrationCredentialStatus.mockResolvedValue({
      ...
      clientId: '456-def.apps.googleusercontent.com',
    });
    const res = await GET(makeRequest());
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.clientId).toBe('456-def.apps.googleusercontent.com');
    expect(mockState.getIntegrationCredentialStatus).toHaveBeenCalledWith('google_seo');
  });
...
describe('POST /api/admin/portal/settings/integrations/google-seo', () => {
  const VALID_BODY = { clientId: '456-def.apps.googleusercontent.com', clientSecret: 'gocspx-real-looking-secret' };
  ...
  it('saves under toolKey "google_seo" with the resolved operator email', async () => {
    const res = await POST(makeRequest(VALID_BODY));
    expect(res.status).toBe(200);
...
describe('TOTP reciente (seguridad, 22/09/2026)', () => {
  it('sin TOTP reciente no deja cambiar el cliente OAuth de Search Console: 403 totp_step_up_required', async () => {
