// =============================================================================
// SEO con IA — unit tests for GET /api/cron/sync-seo-analytics.
//
...
vi.mock('@/lib/seo-analytics-sync', () => ({
...
describe('GET /api/cron/sync-seo-analytics', () => {
  it('401s when CRON_SECRET is not configured on the server', async () => {
    delete process.env.CRON_SECRET;
    const { GET } = await import('@/app/api/cron/sync-seo-analytics/route');
