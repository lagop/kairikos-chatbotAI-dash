...
// =============================================================================
// Fase 2 multi-instancia — el cuerpo de /portal/seo, compartido por las dos
// rutas que lo pintan:
//
//   /portal/seo                      sin id: la única contratación que haya
//   /portal/seo/[clientProductId]    una web concreta
//
// Vive fuera de un page.tsx a propósito, para que las dos rutas sean finas y
// no haya dos copias de 400 líneas que se separen en silencio. Mismo reparto
// que /portal/web.
// =============================================================================
export async function SeoPageBody({
  searchParams,
  clientProductId,
}: {
  /** null = "la única que haya". Con varias y sin id, se ofrece elegir. */
  ...
  // La instancia manda: es la que dice de QUÉ web es todo lo de abajo.
  // Sin id y con varias contrataciones devuelve null, y entonces esta página
  // no puede saber qué enseñar — la ruta índice resuelve ese caso antes de
  // llegar aquí, ofreciendo elegir.
  const instance =
  ...
  return (
    <div className="space-y-6">
        ...
        <section className="card space-y-3" aria-label="Conexión con WordPress" data-testid="seo-wordpress-card">
          ...
          {hasWordPressAppPassword ? (
            <div className="space-y-2">
              ...
              <a href={`/api/portal/seo/wordpress/connect?clientProductId=${instance.clientProductId}`} className="btn-ghost" data-testid="seo-wordpress-reconnect">
                Volver a conectar
              </a>
            </div>
          ) : (
            <a href={`/api/portal/seo/wordpress/connect?clientProductId=${instance.clientProductId}`} className="btn-primary" data-testid="seo-wordpress-connect">
              Conectar WordPress
            </a>
          )}
        </section>
      ...
      {profile ? <SeoKeywordsCard trends={keywordTrends} clientProductId={instance.clientProductId} /> : null}
      ...
      <section className="card space-y-3" aria-label="Search Console" data-testid="seo-search-console-card">
        ...
        {connection?.status === 'active' ? (
          ...
          <div className="space-y-2">
            ...
            <a href={`/api/portal/seo/oauth/start?clientProductId=${instance.clientProductId}`} className="btn-primary" data-testid="seo-search-console-connect">
      ...
      <section className="card space-y-3" aria-label="Google Analytics" data-testid="seo-analytics-card">
        ...
        {analyticsConnection?.status === 'active' ? (
          ...
          <SeoAnalyticsPicker clientProductId={instance.clientProductId} />
        ) : analyticsConnection?.status === 'needs_reconnect' ? (
          <div className="space-y-2">
            <p className="text-sm text-kairikos-danger" data-testid="seo-analytics-status">
              La conexión dejó de funcionar — vuelve a conectarla.
            </p>
            <a href={`/api/portal/seo/analytics/oauth/start?clientProductId=${instance.clientProductId}`} className="btn-primary" data-testid="seo-analytics-connect">
