...
// =============================================================================
// Fase 3 multi-instancia — el cuerpo de /portal/llamadas, compartido por las
// dos rutas que lo pintan:
//
//   /portal/llamadas                    sin id: la única línea que haya
//   /portal/llamadas/[clientProductId]  una línea concreta
//
// Vive fuera de un page.tsx a propósito, mismo reparto que /portal/seo y
// /portal/web: dos copias de 600 líneas se separan en silencio.
// =============================================================================
export async function LlamadasPageBody({
  searchParams,
  clientProductId,
}: {
  /** null = "la única que haya". Con varias, el cuerpo ofrece elegir. */
  ...
  if (view.state === 'pick_line') {
    return (
      <div className="space-y-6">
        <header className="space-y-1">
          <h1 className="text-2xl font-semibold">Llamadas recuperadas</h1>
          <p className="text-sm text-kairikos-muted">Elige la línea que quieres ver.</p>
        </header>
        <ul className="space-y-2">
          {view.lines.map((line) => (
            <li key={line.clientProductId}>
              <Link
                href={`/portal/llamadas/${line.clientProductId}`}
                className="flex items-center justify-between rounded-xl border border-kairikos-border p-4 hover:border-kairikos-accent"
                data-testid="recall-line-option"
              >
                <span className="font-medium">
                  {line.virtualNumber ?? 'Línea sin número asignado todavía'}
                </span>
