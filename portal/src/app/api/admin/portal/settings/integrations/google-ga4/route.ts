...
const DISPLAY_NAME = 'Google Analytics (GA4, SEO)';
...
/**
 * POST /api/admin/portal/settings/integrations/google-ga4
 *
 * Same posture as the google-business route: the DB-backed replacement
 * for GOOGLE_GA4_OAUTH_CLIENT_ID/SECRET, no step-up, no live verification.
 * See lib/google-analytics.ts's resolveClientCredentials().
 */
export async function POST(req: NextRequest) {
  const auth = await authenticateAdminRequest(req);
  if (!auth.ok) return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
  // Seguridad (22/09/2026): el secreto del cliente OAuth de Google: cambiarlo manda el consentimiento de los clientes a otra app. Pide TOTP reciente, no solo sesión.
