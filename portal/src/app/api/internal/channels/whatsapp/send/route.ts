...
// =============================================================================
// Canales — POST /api/internal/channels/whatsapp/send
//
// Mirror of .../telegram/send — the only place the Meta access token is
// decrypted after connect time, and it never leaves this server. n8n
// asks the portal to send the reply on its behalf.
// =============================================================================
...
export async function POST(req: NextRequest) {
  ...
  const result = await sendMessage(token, body.data.phoneNumberId, body.data.to, body.data.text);
  if (!result.ok) {
    logError('channels.whatsapp_send.failed', new Error(result.error), { connectionId: connection.id }, 'warn');
    return NextResponse.json({ ok: false, error: result.error }, { status: 502 });
  }

  return NextResponse.json({ ok: true, messageId: result.data.messages?.[0]?.id ?? null });
}
