...
// =============================================================================
// Canales — POST /api/internal/channels/instagram/send
//
// Mirror of .../messenger/send — the only place the Meta access token is
...
export async function POST(req: NextRequest) {
  ...
  const result = await sendMessage(token, body.data.igUserId, body.data.recipientId, body.data.text);
  if (!result.ok) {
    logError('channels.instagram_send.failed', new Error(result.error), { connectionId: connection.id }, 'warn');
